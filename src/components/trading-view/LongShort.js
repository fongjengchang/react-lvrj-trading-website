      ...
      <div className="tab-panel">
        ...
        <div className={`trailing-section ${mode ? '' : 'not-grey'}`}>
          ...
          <div className={`open-price ${mode ? '' : 'not-grey'}`}>
            <span>
              <FaMinusCircle className="change-value" onClick={() => setLeverage(Number((leverage - 1).toFixed(2)))} />
            </span>
            <input
              type="text"
            ...
            />
            <span>
              <FaPlusCircle className="change-value" onClick={() => setLeverage(Number(leverage) + 1)} />
            </span>
          </div>
          <p className="pos-amount">49.45% Of the Position Amount</p>
          <hr />
          <p>
            <FaRegCircle color="rgba(76, 175, 80, 1)" className="reg-circle-icon" /> Trailing Stop Loss
          </p>
        </div>
        <div className={`trailing-section connect-wall-button ${mode ? '' : 'not-grey'}`}>
          ...
          <a className="button connect-wallet" href="/connect-wallet">
            Connect Wallet
          </a>
        </div>
      </div>
